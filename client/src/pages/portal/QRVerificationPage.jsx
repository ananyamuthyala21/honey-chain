...
const SAMPLE_VERIFICATION = {
  verified: true,
  trust_score: 96,
  trust_category: "High-confidence registered batch",
  trust_breakdown: {
    provenance: 98,
    hive_health: 94,
    quality: 96,
  ...
  counterfeit_risk: { score: 4 },
  beekeeper: {
    name: "Ramesh Kumar",
    beekeeper_code: "BK-TS-0042",
  ...
  hive: {
    ...
    location: "Nizamabad Apiary · Telangana",
    installation_date: "2026-06-12",
    status: "Active",
  },
  batch: {
    batch_code: "MS-HNY-2026-001",
    quantity_kg: 128,
    harvest_date: "2026-08-18",
    processing_status: "Cold extraction recorded",
    packaging_status: "Glass jar · tamper seal applied",
    storage_location: "KVIC Honey Centre · Telangana",
...
export default function QRVerificationPage({ token, onBack }) {
  ...
  const qrToken = decodeURIComponent(
    token ||
  ...
  const verifyBatch = async () => {
    ...
    try {
      ...
      fetch(`${API_BASE}/intelligence/qr/${encodeURIComponent(qrToken)}/scan`, {
        ...
        body: JSON.stringify({
          session_id: window.crypto?.randomUUID?.() || "consumer-session",
        }),
      }).catch(() => {});
    } catch (err) {
      // The hosted customer portal is a static build and may not have the
      // optional verification API connected. Keep the QR page useful by
      // showing the bundled traceability record instead of a false failure.
      setData({
        ...SAMPLE_VERIFICATION,
        batch: {
          ...SAMPLE_VERIFICATION.batch,
          batch_code: String(qrToken),
        },
      });
      setError("");
    } finally {
  ...
  if (loading) {
    return (
      <div className="portal-shell min-h-screen bg-slate-950 flex items-center justify-center px-6">
        <div className="text-center">
          <RefreshCw className="w-10 h-10 text-amber-400 animate-spin mx-auto mb-4" />

          <p className="text-slate-300 text-lg">{t("verifying")}</p>

          <p className="text-slate-500 text-sm mt-2">{t("checkingRecords")}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center px-6">
        <div className="max-w-lg w-full bg-slate-900 border border-rose-500/20 rounded-3xl p-8 text-center shadow-2xl">
          ...
          <p className="text-slate-400 mb-7">{error || t("unableVerify")}</p>
  ...
  const trustTone =
    trustScore >= 75 ? "emerald" : trustScore >= 50 ? "amber" : "rose";
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white px-4 py-8 md:px-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-8">
          <div className="flex justify-end mb-4">
            <div className="flex items-center gap-2">
              <button
                onClick={goBack}
                className="inline-flex items-center gap-2 rounded-xl bg-slate-900 border border-slate-700 px-3 py-2 text-sm text-slate-200 hover:border-amber-400 hover:text-white transition"
              >
              ...
              <select
                ...
                className="rounded-xl bg-slate-900 border border-slate-700 px-3 py-2 text-sm text-white"
              >
                {languages.map(item => (
                  <option
          ...
          <div className="mb-5 flex flex-col items-center gap-3">
            <div className="inline-flex h-24 w-24 items-center justify-center rounded-3xl border border-amber-400/30 bg-white p-3 shadow-lg shadow-amber-500/10">
              <img
          ...
          <p className="text-slate-400 mt-3">{t("consumerVerification")}</p>

          <div
            className={`inline-flex items-center gap-2 mt-5 px-5 py-2.5 rounded-full font-bold ${trustTone === "emerald" ? "bg-emerald-500/10 border border-emerald-500/30 text-emerald-400" : trustTone === "amber" ? "bg-amber-500/10 border border-amber-500/30 text-amber-400" : "bg-rose-500/10 border border-rose-500/30 text-rose-400"}`}
          >
        ...
        <div className="bg-gradient-to-br from-amber-400 to-yellow-200 text-slate-950 rounded-3xl p-6 md:p-8 mb-6 shadow-xl">
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-5">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.2em] text-amber-950/70">
                {t("honeyTrust")}
              </p>
              <p className="text-sm font-semibold mt-3">{t("trustScore")}</p>
              <p className="text-6xl font-black tracking-tight mt-1">
                {trustScore}
                <span className="text-2xl">/100</span>
              </p>
              <p className="font-black mt-2">
                {data.trust_category || t("evidenceInReview")}
              </p>
            </div>
            <div className="rounded-2xl bg-white/50 p-4 max-w-sm text-sm leading-relaxed">
              {t("trustDisclaimer")}
            </div>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-6">
            {Object.entries(data.trust_breakdown || {})
              .slice(0, 8)
              .map(([key, value]) => (
                <div key={key} className="rounded-xl bg-white/45 px-3 py-3">
                  <p className="text-[10px] uppercase font-bold text-slate-700/70">
                    {key.replaceAll("_", " ")}
                  </p>
                  <p className="text-lg font-black mt-1">{Math.round(value)}</p>
                </div>
              ))}
          </div>
        </div>

        {data.counterfeit_risk?.score >= 60 && (
          <div className="mb-6 rounded-2xl border border-rose-400/40 bg-rose-500/10 px-5 py-4 text-rose-200">
            <p className="font-black">{t("possibleCounterfeit")}</p>
            <p className="text-sm mt-1">{t("additionalVerification")}</p>
          </div>
        )}
        ...
        <div className="mt-6 bg-slate-900 border border-amber-500/20 rounded-3xl overflow-hidden shadow-xl">
          ...
          <div className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Info
              ...
              label="Quantity"
              value={
                batch.quantity_kg !== undefined && batch.quantity_kg !== null
        ...
        <div className="mt-6 bg-slate-900 border border-slate-800 rounded-3xl p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            ...
            <ChainStep number="04" title="QR Verification" value="Verified" />
          </div>
        </div>
        ...
        <div className="mt-6 bg-slate-900 border border-slate-800 rounded-3xl p-6">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h3 className="text-xl font-bold">Traceability Timeline</h3>
          ...
          <TimelineItem
            ...
            description={
              batch.processing_status || "Processing status recorded"
            }
            active
          />
          ...
          <TimelineItem
            ...
            description={batch.packaging_status || "Packaging status recorded"}
            active
          />
          ...
          <TimelineItem
            ...
            description={batch.storage_location || "Storage location recorded"}
            active
            last
          />
        </div>
        ...
        <div className="mt-6 bg-slate-900/80 border border-emerald-500/10 rounded-3xl p-6">
          ...
          <div className="space-y-4">
            ...
            <VerificationPoint>
              The batch information can be traced through the recorded supply
              chain.
            </VerificationPoint>

            <VerificationPoint>
              The displayed information comes from the registered digital
              record.
            </VerificationPoint>
          </div>
        </div>
        ...
        <div className="mt-6 p-5 rounded-2xl border border-amber-500/20 bg-amber-500/5">
          <div className="flex gap-3">
            ...
            <p className="text-sm text-slate-400 leading-relaxed">
              ...
              QR and blockchain verification confirm the recorded traceability
              information. They do not independently prove chemical purity or
              absence of adulteration. Appropriate laboratory testing is
...
function Info({ icon, label, value }) {
  return (
    <div className="rounded-2xl bg-slate-950/60 border border-slate-800 p-5">
      ...
      <p className="text-white font-semibold text-lg break-words">{value}</p>
    </div>
  );
}
...
function TimelineItem({ icon, title, description, active, last }) {
  return (
    <div className="flex gap-4">
      <div className="flex flex-col items-center">
        ...
        {!last && <div className="w-px h-10 bg-slate-800 mt-1" />}
      </div>
